...
- レスポンス(`api/blog/get_blog_detail.php`)
    ...
    -いいね切り替え(`api/blog/toggle_like.php`)
        - レスポンス:`success`->true/false, `message`->エラーメッセージ
        - 成功：いいねを追加/削除
        - 失敗：エラーメッセージ表示 
...
#### 運営管理トップページ（`mng.php`）
- 運営管理者のみアクセス可能とすること。
    - 一般ユーザーがアクセスした場合は、ダッシュボードページへリダイレクトすること。
        - `dashboard.php`

- 表示項目
    - ユーザー管理ページへのリンク（`users_mng.php`）
    - ブログ管理ページへのリンク（`blogs_mng.php`）
    - グループ管理ページへのリンク（`groups_mng.php`）

- レスポンス（`api/mng/get_mng_dashboard.php`）
    - `success` -> true/false
    - `message` -> エラーメッセージ
    - `data` -> 運営管理情報
        - ユーザー総数表示        : `user_count`
        - ブログ総数表示          : `blog_count`
        - グループ総数表示        : `group_count`
        - ブロック総数表示        : `block_count`

---

### ユーザー管理ページ（`users_mng.php`）
- 運営管理者のみアクセス可能とすること。
- ユーザー一覧を表示し、ユーザーごとの状態確認や削除、投稿ブログ、所属グループ、ブロック状態を確認できるようにすること。

- レスポンス（`api/mng/get_users.php`）
    - `success` -> true/false
    - `message` -> エラーメッセージ
    - `data` -> ユーザー管理情報
        - ユーザー一覧 : `users[]`
            ...
            - 運営管理者権限      : `is_admin`
            - 投稿ブログ数表示    : `blog_count`
            - 所属グループ数表示  : `group_count`
            - ブロック数表示      : `block_count`

- フォーム
    - ユーザー検索入力 : `user_search`
        - ユーザID、表示名、メールアドレスで検索できること。
    - 状態指定 : `status_filter`
        - 全て
        - 有効ユーザー
        - 停止中ユーザー
...
- 検索ボタン
    - 検索処理（`api/mng/search_users.php`）
        - フォーム項目: `user_search`, `status_filter`, `csrf_token`
        - レスポンス: `success`->true/false, `message`->エラーメッセージ, `users[]`
        - 成功：検索結果を表示
        - 失敗：エラーメッセージ表示
...
#### ユーザー詳細表示
- ユーザー一覧の詳細ボタンから、対象ユーザーの詳細情報を表示すること。

- 詳細取得処理（`api/mng/get_user_detail.php`）
    - フォーム項目: `user_id`, `csrf_token`
    - レスポンス:
        - `success` -> true/false
        - `message` -> エラーメッセージ
        - `data` -> ユーザー詳細情報
            ...
            - 所属グループ一覧    : `groups[]`
                ...
                - 権限表示        : `role`
            - ブロック一覧        : `blocks[]`
                - ブロック対象ユーザID : `blocked_user_id`
                - ブロック対象表示名   : `blocked_display_name`

---

#### ユーザー削除
- ユーザー管理ページから対象ユーザーを削除できること。
- 削除前に確認モーダルを表示すること。
- 運営管理者自身は削除できないこと。
- 削除対象が運営管理者の場合は、削除不可または追加確認を必須とすること。

- ユーザー削除処理（`api/mng/delete_user.php`）
    - フォーム項目: `user_id`, `csrf_token`
    - レスポンス:
        - `success` -> true/false
        - `message` -> エラーメッセージ
...
- 実装上の注意:
    - 物理削除ではなく、可能であれば `is_active = 0` にする停止処理を優先すること。
    - 関連するブログ、コメント、グループ、チャットが存在するため、完全削除する場合は関連データへの影響を確認すること。

---

#### ブロック確認
- ユーザーが誰をブロックしているか、また誰からブロックされているかを確認できること。

- ブロック確認処理（`api/mng/get_user_blocks.php`）
    - フォーム項目: `user_id`, `csrf_token`
    - レスポンス:
        - `success` -> true/false
        - `message` -> エラーメッセージ
        - `data` -> ブロック情報
            - ブロックしているユーザー一覧 : `blocking_users[]`
                - ユーザID       : `user_id`
                - 表示名         : `display_name`
                - アイコン表示   : `icon_url`
            - ブロックされているユーザー一覧 : `blocked_by_users[]`
                - ユーザID       : `user_id`
                - 表示名         : `display_name`
                - アイコン表示   : `icon_url`
    - 成功：ブロック状況を表示
...
#### ブログ削除
- ユーザー詳細から、そのユーザーが投稿したブログを削除できること。
- 削除前に確認モーダルを表示すること。

- ブログ削除処理（`api/mng/delete_blog.php`）
    - フォーム項目: `blog_id`, `csrf_token`
    - レスポンス:
        - `success` -> true/false
        - `message` -> エラーメッセージ
    - 成功：ユーザー詳細情報を再取得して表示
...
- 実装上の注意:
    - 削除対象のブログに紐づくコメント、いいね、関連データも確認すること。
    - 可能であれば、完全削除ではなく非公開化や管理者削除フラグを検討すること。

---

#### グループ削除
- ユーザー詳細から、そのユーザーが作成または所有しているグループを削除できること。
- 削除前に確認モーダルを表示すること。

- グループ削除処理（`api/mng/delete_group.php`）
    - フォーム項目: `group_id`, `csrf_token`
    - レスポンス:
        - `success` -> true/false
        - `message` -> エラーメッセージ
...
- 実装上の注意:
    - 削除対象のグループに紐づくメンバー、チャット、グループブログを確認すること。
    - 完全削除する場合は、関連データも削除するか、表示不可にするかを事前に決めること。